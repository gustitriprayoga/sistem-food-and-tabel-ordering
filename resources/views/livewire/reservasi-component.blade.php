            ...
            <div class="custom-card p-4 p-md-5">
                ...
                <nav class="mb-5">
                    <ol class="breadcrumb justify-content-center" style="--bs-breadcrumb-divider: '>';">
                        <li class="breadcrumb-item {{ $step == 1 ? 'text-accent fw-bold' : 'text-muted' }}">Waktu &
                            Kapasitas</li>
                        <li class="breadcrumb-item {{ $step == 2 ? 'text-accent fw-bold' : 'text-muted' }}">Pilih Meja
                        </li>
                        <li class="breadcrumb-item {{ $step == 3 ? 'text-accent fw-bold' : 'text-muted' }}">Konfirmasi &
                            Bayar</li>
                    </ol>
                </nav>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <input type="number" wire:model.live="jumlah_orang" id="jumlah_orang" min="1"
                                    class="form-control bg-secondary text-light border-0" placeholder="Minimal 1 orang"
                                    required>
                                @error('jumlah_orang')
                                    <span class="text-danger small">{{ $message }}</span>
                                @enderror
                ...
                @if ($step === 2)
                            ...
                            @foreach ($mejaTersedia as $meja)
                                <div class="col-md-4">
                                    <div wire:click="selectMeja({{ $meja->id }})"
                                        ...
                                        <p
                                            class="small {{ $meja_id == $meja->id ? 'text-light fw-bold' : 'text-muted' }}">
                    ...
                    <div class="d-flex justify-content-between mt-4">
                        ...
                        <button wire:click="nextStep" class="btn btn-accent"
                            {{ is_null($meja_id) ? 'disabled' : '' }}>Lanjut Pembayaran &rarr;</button>
                    </div>
                @endif
                ...
                @if ($step === 3)
                    ...
                    <div class="p-4 bg-secondary rounded mb-4">
                        <p class="text-light mb-1">📅 Tanggal/Waktu: <span
                                class="fw-bold text-accent">{{ $tanggal_reservasi }} pukul
                                {{ $waktu_reservasi }}</span></p>
                        <p class="text-light mb-1">👨‍👩‍👧‍👦 Jumlah Orang: <span
                                class="fw-bold">{{ $jumlah_orang }}</span></p>
                        <p class="text-light mb-0">💰 Uang Muka (DP): <span class="fw-bold text-accent">Rp
                                {{ number_format($total_bayar, 0, ',', '.') }}</span></p>
                    </div>

                    <form wire:submit.prevent="checkoutReservasi">
                        <div class="mb-3">
                            <label for="catatan" class="form-label text-light">Catatan (Opsional)</label>
                            <textarea wire:model="catatan" id="catatan" class="form-control bg-secondary text-light border-0" rows="2"
                                placeholder="Contoh: Perlu kursi bayi"></textarea>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="row g-2">
                                @foreach (['transfer_bank' => 'Transfer Bank', 'e_wallet' => 'E-Wallet', 'kasir' => 'Bayar di Kasir (segera)'] as $value => $label)
                                    <div class="col-sm-4">
                                        <input type="radio" wire:model="metode_pembayaran"
                                            value="{{ $value }}" id="metode-{{ $value }}"
                                            class="btn-check" autocomplete="off"
                                            {{ $value === 'kasir' ? 'disabled' : '' }}>
                                        <label
                                            class="btn w-100 {{ $metode_pembayaran == $value ? 'btn-accent text-dark fw-bold' : 'btn-outline-light' }}"
                        ...
                        <div class="d-flex justify-content-between mt-4">
                            <button wire:click="$set('step', 2)" type="button" class="btn btn-outline-light">&larr;
                                Koreksi Meja</button>

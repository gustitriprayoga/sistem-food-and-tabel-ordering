                ...
                @else
                    <div class="mt-4">
                        ...
                        <div class="bg-secondary p-3 rounded">
                            <p class="text-light mb-1">Tipe: <span
                                    class="fw-bold text-accent">{{ $tipePesanan == 'reservasi' ? 'Reservasi Meja (DP)' : 'Pemesanan Menu' }}</span>
                            </p>
                            @if ($tipePesanan == 'reservasi')
                                <p class="text-light mb-1">Tanggal/Waktu: <span
                                        class="fw-bold">{{ $orderData['tanggal'] }} pukul
                                        {{ $orderData['waktu'] }}</span></p>
                                <p class="text-light mb-1">Meja: <span
                                        class="fw-bold">{{ \App\Models\Meja::find($orderData['meja_id'])->nama ?? 'N/A' }}</span>
                                </p>
                            @else
                                <p class="text-light mb-1">Item Menu: <span
                                        class="fw-bold">{{ count($orderData['cart']) }} jenis</span></p>
                                @if ($orderData['meja_id'])
                                    <p class="text-light mb-1">Meja: <span
                                            class="fw-bold">{{ \App\Models\Meja::find($orderData['meja_id'])->nama }}</span>
                                    </p>
                                @else
                                    <p class="text-light mb-1">Tipe Layanan: <span class="fw-bold">Bawa Pulang
                                            (Takeaway)</span></p>
                                @endif
                            @endif
                            <h3 class="mt-3 text-light">Total Pembayaran: <span class="text-accent fw-bold">Rp
                                    {{ number_format($totalBayar, 0, ',', '.') }}</span></h3>
                        </div>
                    </div>
                    ...
                    <form wire:submit.prevent="completeOrder" class="mt-5">
                        <div class="mb-4">
                            ...
                            <div class="row g-2">
                                @foreach (['transfer_bank' => 'Transfer Bank', 'e_wallet' => 'E-Wallet', 'kasir' => 'Bayar di Kasir'] as $value => $label)
                                    <div class="col-sm-4">
                                        <input type="radio" wire:model.live="metode_pembayaran"
                        ...
                        @if ($metode_pembayaran && $metode_pembayaran !== 'kasir')
                            <div class="alert alert-info bg-secondary border-0 text-light small">
                                Silakan transfer total **Rp {{ number_format($totalBayar, 0, ',', '.') }}** ke
                                rekening/wallet kami. Anda akan menerima instruksi lengkap setelah konfirmasi.
                            </div>
                        @endif
